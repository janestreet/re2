...
(** The [sub] keyword argument means, omit location information for subpatterns with index
    greater than [sub].
    ...
    The one exception is for the functions [get_matches], [replace], and [Iterator.next]:
    Since they must iterate correctly through the whole string, they need at least the
    whole match (subpattern 0).  These functions will silently rewrite [~sub] to be
    non-negative.
*)
...
(** [valid_rewrite_template pattern ~template] @return [true] iff [template] is a
    valid rewrite template for [pattern] *)
val valid_rewrite_template        : t -> template:string -> bool

(** [escape nonregex] @return a copy of [nonregex] with everything escaped (i.e.,
...
module Dict : sig
  ...
  val make : ?options:Options.t list -> (string * 'a) list -> 'a redict option
